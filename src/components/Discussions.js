      ...
      try {
        setTimeout(async () => {
          const res = await fetch(
            "https://forum-fullstack.herokuapp.com/api/discussions"
          );
          ...
          if (query.get("search")) {
            const match = data.result.filter((post) => {
              return (
                post.title.toLowerCase().includes(query.get("search")) ||
                post.content.toLowerCase().includes(query.get("search"))
              );
            });
            setPosts(match);
      ...
      <div className="content posts">
        ...
        {filteredPosts &&
          filteredPosts.map((post) => {
            return (
              <div className="discussion" key={post._id}>
                <Link to={`/discussions/${post._id}`}>
                  <h3>{post.title}</h3>
                </Link>
                <p>{post.content}</p>
